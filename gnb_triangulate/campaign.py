"""Join surveyed positions to binocular sightings into a solvable Campaign.
...
from .models import BinocReading, Campaign, Station, SurveyPoint

__all__ = ["CampaignDataError", "build_campaign"]
...
def build_campaign(
    ...
    if duplicates:
        raise CampaignDataError(
            f"survey CSV lists these points more than once: {', '.join(duplicates)}"
        )
    ...
    return Campaign(name=name, points=points)
