...
csv_path = os.path.join(os.path.dirname(__file__), "20260701_hall14_mymaps.csv")
...
# 3. Solved gNB positions (Hall 14 campaign from measurment.xlsx)
...
row_ag["Point Name"] = "trilaterated gnb (Antigravity)"
row_ag["Code"] = "gNB"
...
row_cl["Local time"] = "2026-07-02 11:39:00"
row_cl["Solution Status"] = "FIXED"
row_cl["Measurement Method"] = "Least-squares (Claude implementation)"

# Row 3: Standard named "trilaterated gnb" pointing to the Claude solver result as the primary solution
row_std = row_template.copy()
row_std["Point Name"] = "trilaterated gnb"
row_std["Code"] = "gNB"
row_std["Easting"] = round(e_csv_cl, 4)
row_std["Northing"] = round(n_csv_cl, 4)
row_std["Elevation"] = round(alt_cl, 4)
row_std["Latitude"] = lat_cl
row_std["Longitude"] = lon_cl
row_std["Altitude"] = round(alt_cl, 4)
row_std["Measuring type"] = "Triangulated"
row_std["Local time"] = "2026-07-02 11:39:00"
row_std["Solution Status"] = "FIXED"
row_std["Measurement Method"] = "Least-squares (Claude implementation)"

# 5. Append new rows to dataframe
df_new = pd.concat([df, pd.DataFrame([row_ag, row_cl, row_std])], ignore_index=True)

# 6. Overwrite the original CSV file
df_new.to_csv(csv_path, index=False)

print(f"\nModified CSV saved successfully to: {csv_path}")
